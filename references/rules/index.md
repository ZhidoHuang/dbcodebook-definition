...
| [theme-parameters.json](../databases/theme-parameters.json) | 各数据库用户材料使用的主题色和展示参数 | 文案、定义逻辑和生成流程 |
...
| [definition-task-card.md](../../templates/definition-task-card.md) | 只在确实需要用户裁决时汇总候选方案；可选，不代替探索记录 |
...
5. 主题自身的来源、公式、结果、例外和用户裁决，只留在主题成果与过程证据，不回填公共规则。
6. 模板只在正式规则已经确定需要记录某项事实后增加对应字段，不在模板里另写判断标准。
